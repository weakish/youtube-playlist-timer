<script lang="ts">
  import { calculatePlaylistDuration, extractPlaylistId, filterApiInstance } from "./utils"
  ...
  const sum = async () => {
    timeLength = await calculatePlaylistDuration(extractPlaylistId(playlistId), startIndex - 1, endIndex - 1, apiHost)
  }
</script>
...
<li>Included.</li>
<li>Accessing YouTubde Data API requires an API key, which is not suitable for a staic site.
  Thus, <a href="https://docs.invidious.io/api/">Invidious API</a> is used instead.
  You can choose from the list of available instances, or input your own.
  Invidious is an alternative frontend to YouTube not using official YouTube API. 
</li>
</ol>
